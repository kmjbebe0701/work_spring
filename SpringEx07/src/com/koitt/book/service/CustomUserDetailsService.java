 ...
 * (applicationContext.xml user_service_ref 속성 참고)
 */
...
public class CustomUserDetailsService implements UserDetailsService{
	...
	public UserDetails loadUserByUsername(String email) throws UsernameNotFoundException {
		...
		try {
			/*
			 * 로그인 할 때 클라리언트로부터 전달받은 아이디(email) 값을 넘겨받아 해당 사용자의 정보를 불러온다
			 */
			Users users = service.detailByEmail(email);
			
			// 해당 사용자가 존재하지 않으면 이미 정의된 UsernameNotFoundException을 이용하여 예외를 생성해서 던져주면 스프링이 알아서 예외처리를 하게 된다.
			if (users == null) {
			 ...
			 * authorities
			 */
			return new User(users.getEmail(), users.getPassword(), true, true, true, true, this.getGrantedAuthorities(users));
	...
	private List<GrantedAuthority> getGrantedAuthorities (Users users) {
		 ...
		 * 스프링 4버전 이후부터는 "ROLE_"이라는 문자열을 권한이름 앞에 붙여줘야 정상동작합니다.
		 */
