...
public class FileServiceImpl implements FileService {
	...
	public String add(HttpServletRequest request, MultipartFile attachment) throws FileException {
		...
		try {
			...
			if (attachment != null && !attachment.isEmpty()) {
				...
				/*
				 * 파일명을 데이터베이스에 저장하기 위해 book객체에 파일명을 담는다 (전송시 필요 2)
				 */
	...
	public void download(HttpServletRequest request, HttpServletResponse response, String filename)
		...
		try {
			...
			/*
			 * 파일 캐싱(caching)방지 같은 파일을 내려 받더라도 브라우저가 항상 새로운 파일이라고 인식하기 위해
			 */
			response.setHeader("Pragma", "no-cache");
			response.setHeader("Expires", "-1");

			// OutputStream을 이용하여 읽어들인 파일을 클라이언트(브라우저)로 전송 (커넥션스트림/ BufferedOutputStream체인
			// 스트림)
